...
pub fn compile_components(cargo_artifact_env_key: &'static str, components: &[&'static str]) {
    for component in components {
        ...
        let cwasm = compile_pulley(&wasm_bytes);

        let out_dir = std::env::var_os("OUT_DIR").expect("OUT_DIR missing");
        let out_path = Path::new(&out_dir).join(format!("{component}.cwasm"));
        std::fs::write(&out_path, &cwasm)
            .expect("unable to write pulley compiled artifact to disk");
    }
}
pub fn compile_pulley(wasm_bytes: &[u8]) -> Vec<u8> {
    ...
    // compile wasm component to pulley using wasmtime engine
